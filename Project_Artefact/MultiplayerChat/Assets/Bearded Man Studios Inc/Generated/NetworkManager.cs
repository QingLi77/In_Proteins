...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] DissonanceDemoPlayerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is DissonanceDemoPlayerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (DissonanceDemoPlayerNetworkObject.Length > 0 && DissonanceDemoPlayerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(DissonanceDemoPlayerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<DissonanceDemoPlayerBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateDissonanceDemoPlayer instead, its shorter and easier to type out ;)")]
		public DissonanceDemoPlayerBehavior InstantiateDissonanceDemoPlayerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DissonanceDemoPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<DissonanceDemoPlayerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<DissonanceDemoPlayerBehavior>().networkObject = (DissonanceDemoPlayerNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of DissonanceDemoPlayer
		/// </summary>
		/// <returns>
		/// A local instance of DissonanceDemoPlayerBehavior
		/// </returns>
		/// <param name="index">The index of the DissonanceDemoPlayer prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public DissonanceDemoPlayerBehavior InstantiateDissonanceDemoPlayer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(DissonanceDemoPlayerNetworkObject[index]);
			var netBehavior = go.GetComponent<DissonanceDemoPlayerBehavior>();
